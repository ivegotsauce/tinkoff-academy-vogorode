-- cancel changes of previous change set:
alter table client drop constraint fk_user_type;

alter table client
 	alter column user_type type text using
 		case
 			when user_type = 0 then 'handyman'
 		else 'rancher'
 		end;
...
begin
	...
	loop
		...
		raise notice '% - Committed % rows. Migration paused for % sec.', current_timestamp, total_rows, time_interval;
